...
if __name__ == "__main__":
    ...
    imgs = [transform(Image.open(imgfile).convert("RGB")) for imgfile in args.files]
    imgs = torch.stack(imgs, 0).to(device)

    g_ema = Generator(args.size, 512, 8)
    g_ema.load_state_dict(torch.load(args.ckpt)["g_ema"], strict=False)
    g_ema = g_ema.eval().to(device)
    ...
    noises = [noise.repeat(imgs.shape[0], 1, 1, 1).normal_() for noise in noises_single]
    ...
    for i, input_name in enumerate(args.files):
        ...
        result_file[input_name] = {"img": img_gen[i],
                                   "latent": latent_in[i],
                                   "noise": noise_single}
